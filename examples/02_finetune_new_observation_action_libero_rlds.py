...
python examples/02_finetune_new_observation_action_libero_rlds.py --pretrained_path=hf://rail-berkeley/octo-small-1.5 --data_dir=examples/libero/libero_object/libero_rlds_groups --save_dir=examples/libero/libero_object/finetuned_chkpts

srun -p cscc-gpu-p -q cscc-gpu-qos --gres=gpu:1 --mem=64G --cpus-per-task=8 --pty bash
"""
...
flags.DEFINE_integer("batch_size", 32, "Batch size for finetuning.") #was 128 initially
...
def main(_):
    ...
    dataset = make_single_dataset(
        dataset_kwargs=dict(
            name="libero_rlds_groups",
            data_dir=FLAGS.data_dir,
            image_obs_keys={"primary": "top"},
            proprio_obs_key="robot_obs",
    ...
    config["model"]["observation_tokenizers"].pop("top", None)
    ...
    # You also need to **rename** the key from "robot_obs" → "proprio" in your data batch.
    def process_batch(batch):
        batch = process_text(batch, text_processor)
        batch["observation"]["proprio"] = batch["observation"].pop("robot_obs")
        del batch["dataset_name"]
        return batch
