...
python examples/02_finetune_new_observation_action.py --pretrained_path=hf://rail-berkeley/octo-small-1.5 --data_dir=examples/example_sim_data/aloha_sim_dataset --save_dir=examples/example_sim_data/finetuned_chkpts
python examples/02_finetune_new_observation_action.py --pretrained_path=hf://rail-berkeley/octo-small-1.5 --data_dir=examples/libero/libero_object/rlds --save_dir=examples/libero/libero_object/finetuned_chkpts
"""
...
def main(_):
    ...
    dataset = make_single_dataset(
        ...
        # LIBERO OBJECT HDF5
        dataset_kwargs=dict(
            name="libero_hdf5_converted",  # Match your converted dataset name
            data_dir="examples/libero/libero_object", #the converted rlds dataset
            image_obs_keys={
                "primary": "agentview_rgb",  # Maps to your primary camera
                "secondary": "eye_in_hand_rgb",  # Maps to your wrist camera
            },
            proprio_obs_key="joint_states",  # Using joint states as proprioception
            language_key="language_instruction",
            task_key="task_id",  # Optional: if you want to use task IDs
        ),
        traj_transform_kwargs=dict(
            window_size=1,
            action_horizon=50,
            # Task-specific filtering example:
            filter_functions=[
                ModuleSpec.create(
                    lambda x: "pick_place" in x["task_id"],  # Filter by filename component
                )
            ],
        ),
        frame_transform_kwargs=dict(
            resize_size={"primary": (256, 256), "secondary": (256, 256)},
            # Add any task-specific augmentations
        ),
    ...
    config["model"]["observation_tokenizers"]["proprio"] = ModuleSpec.create(
        ...
        low=-2.0,
        high=2.0,
        # obs_keys=["proprio"], # OCTO SMALL
        obs_keys=["joint_states"], # LIBERO OBJECT HDF5
    )
    # Fully override the old action head with a new one (for smaller changes, you can use update_config)
    config["model"]["heads"]["action"] = ModuleSpec.create(
        L1ActionHead,
        action_horizon=50,
        # action_dim=14, # OCTO SMALL
        action_dim=7, # LIBERO OBJECT HDF5
